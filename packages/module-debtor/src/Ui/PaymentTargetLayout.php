...
/**
 * Layout config for a host that mounts the Zahlungsziele fragment (ADR-018
 * pattern, the tax-code / chart model): the host's
 * `Ui/Config/Finance/paymentTargetControllerConfig.inc.php` delegates here (one line) and pins the
 ...
 */
final class PaymentTargetLayout
{
        ...
        return [
            'levelElements' => [
                'body' => [
                    'main' => [[
                        'nameSpace' => self::NS,
                        'path'      => 'Backend/PaymentTargetController',
                        'name'      => 'listAction',
                    ]],
                ],
            ],
