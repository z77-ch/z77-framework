...
- When surfacing FAILURE feedback → the handler owns it and returns it: per-field `errors` plus a form-level `formError` banner (`form.error.check` / `form.error.csrf` / `form.error.send`), both through `viewContext()`. MUST NOT re-implement per-project failure texts; override the wording via `withMessageKeys()` or the i18n keys.
