...
- **Rates: add, never edit; backdating refused; removal narrow** (`VatMasterData::addRate()` / `removeRate()`, owner 2026-09-21). `addRate()` stamps `createdOn` with today, runs `TaxRateValidator` with today and persists. The validator refuses a `validFrom` before today — EXCEPT when it lies before the code's earliest existing row (pure backfill, e.g. pre-2018 rates for a migration; a code with no rows yet has no «earliest», so its first rate cannot be backdated). A row may be removed while its validity has not started, or when it was entered TODAY for today (the same-day typo — `validFrom === createdOn === today`); every other row in effect stays (`RateInEffectException`). Seed rows carry no `createdOn` and are therefore never the same-day case.
...
- When a rate changes (a new law) → MUST add a NEW row with `validFrom` today or later («Neuer Satz gültig ab» in the backend); MUST NOT backdate it into days existing rows cover — a `validFrom` before today is accepted only before the code's earliest row (backfilling history)
...
- When referencing a tax code from a Doctrine entity (invoice line, journal line) → MUST store the `code` string plus the snapshot fields (`rate`, `label`), never a numeric id of the file-based row
...
- **VAT-RATE-001** — resolved 2026-09-21 (owner decision): a same-day typo is repairable in the backend — a row entered today for today can be removed today (`createdOn`); every other row in effect stays, and a wrong rate that applied is corrected by a new row from today plus credit notes. Don't assume a backdated correction is possible: `validFrom` before today is refused unless it backfills before the code's earliest row.
- **VAT-RATE-002** — don't assume the first rate of a NEW code can start in the past: with no existing row there is no «earliest» to backfill before, so `addRate()` refuses any `validFrom` before today. An installation that creates a code for documents with older service dates enters it with `validFrom` today; older documents must use a code that existed then.
...
- **Snapshot serialisation and code picker (P3, with debtor)**: `ResolvedRate` / `TaxSummary` ↔ array (`toArray()` / `fromArray()`, ints only, float refused) and an «active codes for a new document» query were built and REMOVED on review (no production caller). They come back with the invoice that stores them and the line editor that picks them.
