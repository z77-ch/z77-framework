...
SOURCE=/packages/kernel/shared/src/Content/InlineProfile.php
SOURCE=/packages/kernel/shared/src/Content/BlockSchemaValidator.php
...
SOURCE=/packages/kernel/shared/src/Content/PageContent.php
SOURCE=/packages/kernel/shared/src/Services/ContentVariantService.php
SOURCE=/packages/module-backend/res/view/templates/Content/ContentController/confirmRestore.tpl.php
SOURCE=/tests/content-variants.php
SOURCE=/tests/content-page-editor.php
SOURCE=/packages/module-backend/res/view/templates/html-bare-skeleton.tpl.php
SOURCE=/packages/module-backend/res/assets/js/content/slot.js
SOURCE=/packages/module-frontend/res/assets/js/content-edit.js
...
- Inline formatting lives only inside a block's text value, via `InlineMarkdown` (whitelist `**bold**`, `*italic*`, `[label](url)`, and `break` → `<br>` under a profile). Escape-first, then introduce whitelisted tags. Without a profile (legacy path) bold, italic and links apply to every field; with an `InlineProfile` (schema-aware path, below) only what the field allows. Protocol-relative targets (`//host`) are never links.
- **Blueprints (ADR-044, [`../02-decisions/adr-044-content-blueprints.md`](../02-decisions/adr-044-content-blueprints.md))** — a fixed, keyed block structure per slug, for designed pages the client may fill but not restructure:
    - Declared in code, in a module config: `contentBlueprints` (slug ⇒ ordered slots `{key, type, label}`), next to `contentBlocks`. Project actions for links: `contentActions` (name ⇒ `{href, attributes}`). Both collected by `ContentExtensions::assemble()` (not DI, like `BlockRegistry`); a slug or action declared twice throws.
    - Blocks carry an optional `key`; slots bind by key AND type. `Blueprint::arrange()` lays stored blocks out per slot for the editor (fresh block from schema defaults for an empty slot); `Blueprint::enforce()` is what gets saved — one block per slot in slot order, `type`/`key` from the slot, fields from the post, orphans (key in no slot, or type mismatch) from the STORED document only.
...
- **`PageContent::view($slug)` — the page entry point (2026-09-22).** A designed page (bespoke, blueprint) reads its document through `Z77\Shared\Content\PageContent::view()`: request language, the content preview (`ContentPreview::key()`), and an exception when the document is missing or inactive (a designed page without its text is a deploy error; an empty page would hide it). `ContentService` itself stays request-free. Moved here from the first consumer (zihlundsee.ch), where it had been project code.
- **Variants and preview (ADR-044 addendum 2026-09-22)** — a text release without a release mechanism:
    ...
    - A preview request is never page-cached (`PageCachePolicy`, explicit) and gets `noindex, nofollow` (frontend `partials/head/meta`). The frontend skeleton has a body slot `preview`, filled by the framework partial `partials/previewBar` (layoutConfig body element `preview`): a pill «Vorschau «key» · Live ansehen», the live link built by `ContentPreview::withoutPreview()`. Class names `preview-bar*` are the framework's, the default look sits in `components/_preview-bar.scss` (`messages` sheet) and a project restyles them — same split as the flash messages. A project that overrides `layoutConfig` must keep the `preview` entry.
    - **Publish** (backend, `ContentVariantService::publish($key, $username)`): per document of the set the version rule below runs (previous live copy → version, one `v-…` key for the whole run), the variant becomes live, the variant file is removed. Not atomic across files. Variants and versions stay until an admin deletes them. (Before ADR-045 the previous live copies went into one archive SET `alt-YYYYMMDD-HHMM`; existing `alt-…` sets are ordinary sets and can still be published.)
- **Versions on every live save (ADR-045 §3, [`../02-decisions/adr-045-editor-role-and-frontend-editing.md`](../02-decisions/adr-045-editor-role-and-frontend-editing.md))** — one rule for every write of a live copy:
    - `ContentVariantService` is the ONE place a live copy is written: `saveLive($content, $username)` (backend save/add/active switch, the page editor `slotAction`), `publish()`, `restore($version, $username)` all end in the private `writeLive()`. It archives the STORED live copy as a one-document variant `v-YYYYMMDD-HHMMSS` (`-2`, `-3` … while that document already has the key; `ContentPreview::versionKey()` / `isVersionKey()`, `Content::isVersion()`), keeping ITS `changedBy`/`changedAt`, then writes the new live copy stamped with the saving user and ISO time. A first save (no live copy yet) archives nothing.
    - `Content::changedBy` (username) / `changedAt` (ISO-8601, `DATE_ATOM`): the state's author, not the archiver. `''` = unknown (documents from before ADR-045; variants — variant saves and `createVariant()` write no stamp and no version). Server-controlled: `ContentController::edit()` forces the stored values back after `mapFromArray()`.
    - Restore = `restore()` of ONE version: it becomes live (stamped with the restorer), the replaced live copy becomes a new version, the restored version file is removed. Other documents of the same publish run are not touched; publishing the version key as a set restores the whole run.
    - A version is history: the backend does not edit it or switch its `active` flag (refused server-side, not offered in the UI) — only «Vorschau öffnen», «Wiederherstellen» and, for ADMIN, «Löschen».
    - Backend list: per slug the live row («zuletzt gespeichert <Datum> von <user>»), then variants by key, then versions newest first («Version 22.09.2026 15:30 · anna»; a version without stamp reads «älterer Stand, gesichert <time from key>»). The edit modal header shows the live copy's last save.
- **Editing on the page (ADR-045 §4)** — an editor edits a page's text where it is shown, one blueprint slot at a time:
    - **Marker.** A template prints `ContentView::editAttribute($slot)` on the ROOT element of the markup that renders that slot: `<section class="intro"<?= raw($page->editAttribute('intro')) ?>>`. It returns ` data-content-edit="<slot editor URL>" data-content-edit-label="<slot label>"` (leading space, escaped) — or `''`. `''` for everyone but a user with at least `editor` on a full page (`PageContent::view()` decides per request from the logged-in user and hands the view `forEditor()`; a fetch fragment never gets markers), for a key that is no slot of the slug's blueprint, for a slug without blueprint, for a shown version and for a version preview key. So a visitor's HTML is byte-identical with or without markers. A controller that maps slot values into partial data passes the marker along (e.g. `'intro' => [..., 'edit' => $page->editAttribute('intro')]`) and the partial prints it on its root; a slot without a marker simply gets no button.
    - **What the marker names:** the document actually SHOWN — slug, language, variant after language fallback and preview (a French page showing the German `legal` edits `legal.de`) — plus the slot and the request's `preview` key. URL: `ContentView::slotEditorUrl()` → `/backend/content/content/slot?slug=&language=&variant=&slot=[&preview=]`.
    - **The button.** `AbstractFrontendController::html()` adds `content-edit.js` (module-frontend) for the same users on full pages. It puts one «Bearbeiten» button per marker into its own fixed layer over the element's top-right corner (never inside the element: no layout shift, no new positioning context; two buttons sharing a corner are moved apart), keyboard-reachable, `aria-label` «Bearbeiten: <label>», a dashed outline of the slot on hover/focus. The script injects its own `<style>` (`z77-ce-*`, `all: unset`) — a project need not load a framework stylesheet.
    - **The editor** opens in an `<iframe>` in a modal: `ContentController::slotAction` renders the backend's own blueprint editor (`edit.tpl.php` in slot mode: that one slot card, no title/active/slug) on a bare backend page (`html-bare-skeleton`: backend CSS, icon sprite, csrf meta, core.js — no topbar, no menu; the site's CSS cannot reach it). Close = ×, Esc (also inside the iframe, `content/slot.js` forwards it), «Abbrechen» (message `z77:content-edit-close`); not a backdrop click (POPUP-CLOSE-001).
    - **Save.** The POST merges the posted slot into the STORED blocks with `Blueprint::enforceSlot()` — every other slot and every orphan from the store, so a crafted body cannot change them — then the validator, the optimistic lock (`entity_hash` of the stored document) and the save rule: a shown **variant** is saved as that variant (no version, no stamp); a **live copy shown in a preview** goes into the preview set (the set's copy if it has one by now, else a new variant = stored live copy + this slot — the live file is not written); a **live copy without preview** goes through `ContentVariantService::saveLive()` (version rule). Versions are refused (404 on GET, error on POST). Title, `active`, identity and stamps are never taken from the post. Success answers with the core command `post-message` `{type: 'z77:content-saved'}`; `content-edit.js` accepts it only from its own origin and from the open iframe's window and reloads the page — what you then see is what the server renders. A failed save re-renders the form in place (the bare page's body is the popup channel's root).
    - **Access:** `slotAction` is EDITOR like the rest of the content editor (backendConfig); a guest is redirected to the login. **Framing:** same-origin iframe; the framework sends no `X-Frame-Options` / CSP `frame-ancestors` (security.md pending). When those headers come, the slot editor must stay frameable by its own origin (`SAMEORIGIN` / `frame-ancestors 'self'`) — or the button opens an empty frame.
...
- When a designed page must keep its structure → MUST declare a blueprint under `contentBlueprints` in a module config (code, ships with the release); MUST NOT store structure in `data/` (it must agree with the templates and CSS of the same release) (ADR-044)
- When saving a blueprint document → MUST pass the posted blocks through `Blueprint::enforce()` with the STORED blocks; MUST NOT take orphans or extra blocks from the post (a crafted body could add or remove sections)
...
- When a designed frontend page reads its content document → MUST use `PageContent::view($slug)` (language + preview + loud failure); a direct `ContentService::view()` call MUST pass `ContentPreview::key()` itself, or the page silently ignores previews. MUST NOT read `?preview=` anywhere else or remember it in a cookie/session (the URL is the only carrier)
...
- When writing a LIVE content document (any caller: backend, frontend editor, a script) → MUST go through `ContentVariantService::saveLive()` / `publish()` / `restore()`; MUST NOT `persist()` a live `Content` directly — it would skip the version and the stamp (ADR-045 §3)
- When hydrating a `Content` from a request body → MUST force `changedBy`/`changedAt` back to the stored values (BodyCleaner passes them through); only `ContentVariantService` sets them
- When saving a VARIANT → MUST `persist()` it directly (no version, no stamp); MUST NOT route it through `saveLive()` (throws `LogicException`)
- When a designed page template renders a blueprint slot → SHOULD print `ContentView::editAttribute($slot)` (raw, it is escaped) on the ROOT element of that slot's markup, passed down from the controller with the slot's data; MUST NOT compute the role or the request mode in the template (the view decides), and MUST NOT print a marker for a key that is not a slot of the document's blueprint (it renders `''` anyway)
- When the page editor saves a slot → MUST merge with `Blueprint::enforceSlot()` against the STORED blocks; MUST NOT take other slots, orphans, title, `active` or stamps from the post; a live copy shown in a preview MUST go into the preview set, never live
- When a page is shown inside an iframe and must tell its parent something → MUST use the core command `post-message` (target origin = own origin); the receiver MUST check `event.origin`, `event.source` and the message `type`
- When a content action removes a document, variant or version → it MUST stay ADMIN in the access config (`confirmDeleteAction`/`removeAction`); every other content action is EDITOR (ADR-045 §1)
...
- [`../02-decisions/adr-044-content-blueprints.md`](../02-decisions/adr-044-content-blueprints.md) — blueprints, keyed blocks, field profiles, action links, localise flag
...
- ~~**Keyed blocks (named bespoke slots)**~~ — DONE 2026-09-21 with ADR-044 (`key` on blocks, `Content::keyed()`, `ContentView::keyed()`, blueprints). Original note: bespoke access is by-type (`Content::block($type)`); a repeated type (e.g. two `prose-section`s on `/about`) can only be reached via `blocks($type)` (list) or index, not a unique name. If named sections are wanted (`block('mission')` vs `block('standort')`), design a keyed-block concept (an optional `key`/`slot` field on a block + a `Content::keyed($key)` accessor) or use distinct types per role. Not blocking — surfaced by the `/about` bespoke build.
- **Blueprints follow-ups (ADR-044)** — (a) backend list: mark blueprint documents and offer creating a document for a blueprint slug that has none; (b) file-level pull/push tooling with checksum for editing outside the backend (first built in the zihlundsee.ch project); (c) page tree in the backend list. Branch `feat/content-blueprints`, bauplan [`../03-development/content-blueprints-bauplan.md`](../03-development/content-blueprints-bauplan.md).
