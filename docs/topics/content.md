...
- **Blueprints (ADR-044, [`../02-decisions/adr-044-content-blueprints.md`](../02-decisions/adr-044-content-blueprints.md))** — a fixed, keyed block structure per slug, for designed pages the client may fill but not restructure:
    ...
    - Descriptors gain `required`, `maxLength` (scalars), `min`/`max` (lists), `inline` (`bold|italic|link|break`) and `links` (`targets` `page|media|external|mailto|action`, `localize`). `ContentValidator` checks values against the schema via `BlockSchemaValidator`.
