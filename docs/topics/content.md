...
- **Versions on every live save (ADR-045 §3, [`../02-decisions/adr-045-editor-role-and-frontend-editing.md`](../02-decisions/adr-045-editor-role-and-frontend-editing.md))** — one rule for every write of a live copy:
    - `ContentVariantService` is the ONE place a live copy is written: `saveLive($content, $username)` (backend save/add/active switch, and the frontend slot editor to come), `publish()`, `restore($version, $username)` all end in the private `writeLive()`. It archives the STORED live copy as a one-document variant `v-YYYYMMDD-HHMMSS` (`-2`, `-3` … while that document already has the key; `ContentPreview::versionKey()` / `isVersionKey()`, `Content::isVersion()`), keeping ITS `changedBy`/`changedAt`, then writes the new live copy stamped with the saving user and ISO time. A first save (no live copy yet) archives nothing.
