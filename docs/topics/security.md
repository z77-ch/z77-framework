...
- **SEC-003** (open): don't assume session cookies are hardened — `SessionManager` calls `session_start()` without explicit `Secure` / `HttpOnly` / `SameSite` cookie params (relies on php.ini defaults).
...
- **Session cookie flags** (fixes SEC-003) — `Secure` (HTTPS) + `HttpOnly` + `SameSite=Lax` via `session_set_cookie_params`.
