...
- **SEC-006** (open): don't assume an installation that predates 2026-08-07 is covered by SEC-005. `canonicalBaseUrl` defaults to empty, and empty still falls back to the `Host` header — the framework cannot invent a hostname, and refusing to serve would break every existing installation. The fallback logs once per request via `error_log`. Set `extra.core-bootstrap.canonicalBaseUrl` in the project's `composer.json` and re-run `composer install`; `config/bootstrap.inc.php` is regenerated on every install, so a value written into that file directly is lost.
...
- **Canonical base URL in every project** (closes SEC-006) — the mechanism exists; each installation still has to set `extra.core-bootstrap.canonicalBaseUrl`. Optional hardening on top, deliberately not built yet: reject a request whose `Host` differs from the canonical one (400/421). That is defense-in-depth against further Host tricks, but it breaks apex-vs-www and staging setups, so it needs its own opt-in switch and a migration story.
