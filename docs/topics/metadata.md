...
SOURCE=/packages/kernel/core/src/Libraries/Seo/SiteIdentity.php
...
- **Site identity (`$site`, 2026-09-22)** — what is the same on every page of a module and therefore NOT per-page metadata: the site name (`og:site_name`, `<title>` fallback), the author (`<meta name="author">`), the Open Graph locale per language and the link-preview image (+ size, alt per language). Declared in the module config block `site` (a project overrides `<module>Config.inc.php`); `AbstractBaseController::html()` builds a `SiteIdentity` for the request language and injects it as `$site` (an `empty()` one when the block is absent). The head partials print from `$metaData` (per page) and `$site` (per module) only — **a missing value leaves its tag out**. The starter used to print "Max Mustermann", "Portfolio von Max Mustermann" and example.com into every page of every site that did not override `meta`/`social`, and `<title>` fell back to "Z77 Framework" on every page without metadata (a PRG target, a legal page). Example block: see the class comment of `SiteIdentity`.
...
- **Site-wide crawl block (SEO-NOINDEX-001)** is a SEPARATE axis from per-page `MetaData` — a single deployment-level switch (staging / pre-launch) that keeps the WHOLE site out of search engines. Source of truth is the flag file `var/state/noindex.flag` (release-local since ADR-035 — before that it was `data/framework/seo/noindex.flag`, which is shared across release doors and could therefore not be set on staging alone) → the `SEO_NOINDEX` constant (defined in `Bootstrap::__construct()` next to `DEBUG`, see [`bootstrap.md`](bootstrap.md)). When active, the frontend `head/meta.tpl.php` emits `<meta name="robots" content="noindex, nofollow">` and the backend shows a persistent Störer (`.be-shell-banner`). Toggled via the filesystem OR the backend service panel (`SystemController::toggleNoindexAction`, clears APCu + PageCache). There is NO per-page `noindex` field on `MetaData` yet.
...
- When a navigation entry has no metadata or the route is a convention route (no entry) → `$metaData` is `null`; head partials MUST fall back to the site identity (`$metaData?->getTitle() ?: $site->name`); MUST NOT print a fixed placeholder (a framework or person name, example.com).
- When a head tag describes the site rather than the page (author, site name, og:locale, preview image) → MUST come from the module config `site` via `$site`; a project MUST NOT override `head/meta` or `head/social` just to set these. A missing entry MUST leave the tag out.
