...
- **Snapshot shape (P3, with debtor)**: how a document stores the address it used (which fields, JSON column or flat columns) arrives with the invoice that stores it; this module carries no serialisation of its own until then. The same for `AddressTypes::resolve(code)` returning the row (with its own exception for an unknown code): removed on review 2026-09-21 for want of a production caller, comes back with the consumer that snapshots the type.
