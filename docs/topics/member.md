...
- **Several accounts on one project reference, always by invitation from the MASTER** (B7 v1.1.0, `InvitationFlow`): the first account of a reference — the registration we activated — is its master (`MemberAccount::$tenantRole`); it invites by address, pauses and removes. An invited account is born `confirmed` with the reference set (the click in the inbox was the verification), waits for our activation like any registration, and can do everything the master can EXCEPT invite. «Zugänge» is an AREA of the shell (`ZugaengeController`, nav slot `member-main`) since 2026-09-12 — until then the profile's fourth section — and it exists only for the master standing on his HOME (`InvitationFlow::managesHere()`): «not present, not forbidden», dropped from switcher and rail by `AbstractMemberController::addAreas()` and answered with a silent redirect by the controller. It moved because the accesses belong to the reference, the profile to the person: with a switched header the section listed the home's accounts under another reference's name.
...
- When a project wants the «Zugänge» area → MUST add a nav entry with controller `zugaenge` / action `index` in slot `member-main` (installation DATA, per machine), MUST carry the `ZugaengeController` block in its whole-file `memberConfig` override, and MUST re-ship the `shell.js` hand copy (the pause switch posts to the area's route). Visibility is not the project's to decide: `addAreas()` drops the entry unless `managesHere()` holds, and the WRITES still act on the home through `mayManage()` — a page that showed the home must finish what it started even if another tab switched the session.
...
- [`../02-decisions/adr-038-member-knows-no-project-reference.md`](../02-decisions/adr-038-member-knows-no-project-reference.md) — **decided 2026-09-13, not yet built:** the module keeps the person and the door; `company`, `tenantRef`, `tenantRole` and `grants.json` leave it, memberships live at the project's tenant, the module asks through a hook. Supersedes ADR-037's data home; the MEM rules that read `getTenantRef()` for ownership retire with the build.
