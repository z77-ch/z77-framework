...
- **Set `canonicalBaseUrl` per installation** — the mechanism landed with SEC-005, but an installation that has not set `extra.core-bootstrap.canonicalBaseUrl` still falls back to the `Host` header for its mail links (SEC-006).
