...
- **Anti-oracle is a design constraint, not a feature:** register, resend and login answer the same page for known, unknown and throttled addresses — the mail differs, the page never does. A waiting record is opened even when no link went out.
...
- When adding a mail or page to a login/registration path → MUST keep the visitor-facing answer identical for known, unknown and throttled addresses; the differences belong in the mail body only.
