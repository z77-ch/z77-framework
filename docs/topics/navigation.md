...
SOURCE=/tests/navigation-subpage-cursor.php
...
- **Sub-page cursor (since 2026-08-12, NAV-SUBPAGE-001):** a page whose 4-tuple has NO entry — a sub-page of a listed controller, e.g. backend `estate/tree` beside the listed `estate/list` — used to leave the UI without any cursor. `getActiveSectionBySlot()` then found no section and `partials/subnav` returned early: **the whole left column vanished.** `resolveCurrent()` now sets a fallback cursor to the first ACTIVE entry sharing `module`+`group`+`controller`, and `resolveUiCurrent()` starts from it. The fallback lands on `$uiCurrent` ONLY — never on `$current`, because `$current` drives the SEO metadata and the canonical path (`AbstractBaseController`), and a sub-page must not inherit its sibling's canonical URL. It fires only where the answer was previously «nothing», so no existing resolution changes. Inactive siblings are skipped (`iterateTree()` skips them too, so the section would stay empty anyway); a controller with no entry at all still gets no cursor — nothing is guessed. Test: `tests/navigation-subpage-cursor.php`.
